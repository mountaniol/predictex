...
def break_cycles(graph: Dict[str, Set[str]]) -> Dict[str, Set[str]]:
    """Разрывает циклы в графе, удаляя наименее важные зависимости"""
    ...
    # Разрываем циклы, удаляя зависимости в обратном порядке
    for cycle in cycles:
        if len(cycle) > 1:
...
def reorder_questions(questions: List[Dict]) -> List[Dict]:
    ...
    # Разрываем циклы
    broken_graph = break_cycles(graph)
